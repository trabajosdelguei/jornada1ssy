...
function CurrentProjectID() {
	...
	return "{7D17B05A-1E09-4241-BCB8-DFDB2802E5B9}";
}
...
class AdvancedSecurity
{
	...
	public function validateUser(&$usr, &$pwd, $autologin, $provider = "")
	{
		...
		if (!$valid) {
			...
			if ($rs = $UserTableConn->execute($sql)) {
				if (!$rs->EOF) {
					$valid = $customValid || ComparePassword($rs->fields('contrasenia'), $pwd);
					if ($valid) {
						$this->_isLoggedIn = TRUE;
						$_SESSION[SESSION_STATUS] = "login";
						$_SESSION[SESSION_SYS_ADMIN] = 0; // Non System Administrator
						$this->setCurrentUserName($rs->fields('nombrecompleto')); // Load user name
						if ($rs->fields('userlevel_id') == NULL) {
							$this->setSessionUserLevelID(0);
						} else {
							$this->setSessionUserLevelID((int)$rs->fields('userlevel_id')); // Load User Level
						}
						$this->setupUserLevel();
						...
						$UserProfile->delete('contrasenia'); // Delete password
	...
	{
		return $this->currentUserInfo("correo");
	}
