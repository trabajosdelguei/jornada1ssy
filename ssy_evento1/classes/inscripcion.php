...
/**
 * Table class for inscripcion
 */
class inscripcion extends DbTable
{
	protected $SqlFrom = "";
	protected $SqlSelect = "";
	...
	public function __construct()
	{
		...
		$this->TableVar = 'inscripcion';
		$this->TableName = 'inscripcion';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "`inscripcion`";
		$this->Dbid = 'DB';
		$this->ExportAll = TRUE;
		...
		// idinscripcion
		$this->idinscripcion = new DbField('inscripcion', 'inscripcion', 'x_idinscripcion', 'idinscripcion', '`idinscripcion`', '`idinscripcion`', 3, -1, FALSE, '`idinscripcion`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		$this->idinscripcion->IsAutoIncrement = TRUE; // Autoincrement field
		$this->idinscripcion->IsPrimaryKey = TRUE; // Primary key field
		$this->idinscripcion->Sortable = TRUE; // Allow sort
		$this->idinscripcion->DefaultErrorMessage = $Language->phrase("IncorrectInteger");
		$this->fields['idinscripcion'] = &$this->idinscripcion;
	...
	{
		return ($this->SqlFrom <> "") ? $this->SqlFrom : "`inscripcion`";
	}
	...
	public function insert(&$rs)
	{
		...
		if ($success) {
			...
			$this->idinscripcion->setDbValue($conn->insert_ID());
			$rs['idinscripcion'] = $this->idinscripcion->DbValue;
		}
		return $success;
	}
	...
	protected function deleteSql(&$rs, $where = "", $curfilter = TRUE)
	{
		...
		if (is_array($where))
			$where = $this->arrayToFilter($where);
		if ($rs) {
			if (array_key_exists('idinscripcion', $rs))
				AddFilter($where, QuotedName('idinscripcion', $this->Dbid) . '=' . QuotedValue($rs['idinscripcion'], $this->idinscripcion->DataType, $this->Dbid));
		}
		$filter = ($curfilter) ? $this->CurrentFilter : "";
		AddFilter($filter, $where);
		if ($filter <> "")
	...
	public function getRecordFilter($row = NULL)
	{
		...
		$val = is_array($row) ? (array_key_exists('idinscripcion', $row) ? $row['idinscripcion'] : NULL) : $this->idinscripcion->CurrentValue;
		if (!is_numeric($val))
			return "0=1"; // Invalid key
		if ($val == NULL)
			return "0=1"; // Invalid key
		else
			$keyFilter = str_replace("@idinscripcion@", AdjustSql($val, $this->Dbid), $keyFilter); // Replace key value
		return $keyFilter;
	}
	...
	public function getModalCaption($pageName)
	{
		...
		elseif ($pageName == "inscripcionedit.php")
			return $Language->phrase("Edit");
		elseif ($pageName == "inscripcionadd.php")
	...
	{
		if ($parm <> "")
			$url = $this->keyUrl("inscripcionview.php", $this->getUrlParm($parm));
		else
			$url = $this->keyUrl("inscripcionview.php", $this->getUrlParm(TABLE_SHOW_DETAIL . "="));
	...
	{
		if ($parm <> "")
			$url = "inscripcionadd.php?" . $this->getUrlParm($parm);
		else
			$url = "inscripcionadd.php";
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("inscripcionedit.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("inscripcionadd.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		return $this->keyUrl("inscripciondelete.php", $this->getUrlParm());
	}
	...
	{
		$json = "";
		$json .= "idinscripcion:" . JsonEncode($this->idinscripcion->CurrentValue, "number");
		$json = "{" . $json . "}";
		if ($htmlEncode)
	...
	public function keyUrl($url, $parm = "")
	{
		...
		if ($parm <> "")
			$url .= $parm . "&";
		if ($this->idinscripcion->CurrentValue != NULL) {
			$url .= "idinscripcion=" . urlencode($this->idinscripcion->CurrentValue);
		} else {
	...
	public function getRecordKeys()
	{
		...
		if (Param("key_m") !== NULL) {
			...
			if (Param("idinscripcion") !== NULL)
				$arKeys[] = Param("idinscripcion");
	...
	public function getFilterFromRecordKeys()
	{
		...
		foreach ($arKeys as $key) {
			...
			$this->idinscripcion->CurrentValue = $key;
	...
	public function renderEditRow()
	{
		...
		// idinscripcion
		$this->idinscripcion->EditAttrs["class"] = "form-control";
		$this->idinscripcion->EditCustomAttributes = "";
		$this->idinscripcion->EditValue = $this->idinscripcion->CurrentValue;
		$this->idinscripcion->ViewCustomAttributes = "";
