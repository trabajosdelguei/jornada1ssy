...
class usuarios extends DbTable
{
	...
	public function __construct()
	{
		...
		// idusuarios
		$this->idusuarios = new DbField('usuarios', 'usuarios', 'x_idusuarios', 'idusuarios', '`idusuarios`', '`idusuarios`', 3, -1, FALSE, '`idusuarios`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		$this->idusuarios->IsAutoIncrement = TRUE; // Autoincrement field
		$this->idusuarios->IsPrimaryKey = TRUE; // Primary key field
		$this->idusuarios->Sortable = TRUE; // Allow sort
		$this->idusuarios->DefaultErrorMessage = $Language->phrase("IncorrectInteger");
		$this->fields['idusuarios'] = &$this->idusuarios;

		// nombrecompleto
		$this->nombrecompleto = new DbField('usuarios', 'usuarios', 'x_nombrecompleto', 'nombrecompleto', '`nombrecompleto`', '`nombrecompleto`', 200, -1, FALSE, '`nombrecompleto`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->nombrecompleto->Nullable = FALSE; // NOT NULL field
		$this->nombrecompleto->Required = TRUE; // Required field
		$this->nombrecompleto->Sortable = TRUE; // Allow sort
		$this->fields['nombrecompleto'] = &$this->nombrecompleto;

		// correo
		$this->correo = new DbField('usuarios', 'usuarios', 'x_correo', 'correo', '`correo`', '`correo`', 200, -1, FALSE, '`correo`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// contrasenia
		$this->contrasenia = new DbField('usuarios', 'usuarios', 'x_contrasenia', 'contrasenia', '`contrasenia`', '`contrasenia`', 200, -1, FALSE, '`contrasenia`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->contrasenia->Nullable = FALSE; // NOT NULL field
		$this->contrasenia->Required = TRUE; // Required field
		$this->contrasenia->Sortable = TRUE; // Allow sort
		$this->fields['contrasenia'] = &$this->contrasenia;

		// userlevel_id
		$this->userlevel_id = new DbField('usuarios', 'usuarios', 'x_userlevel_id', 'userlevel_id', '`userlevel_id`', '`userlevel_id`', 3, -1, FALSE, '`userlevel_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
	...
	protected function insertSql(&$rs)
	{
		...
		foreach ($rs as $name => $value) {
			if (!isset($this->fields[$name]) || $this->fields[$name]->IsCustom)
				continue;
			if (ENCRYPTED_PASSWORD && $name == 'contrasenia')
	...
	public function insert(&$rs)
	{
		...
		if ($success) {
			...
			$this->idusuarios->setDbValue($conn->insert_ID());
			$rs['idusuarios'] = $this->idusuarios->DbValue;
		}
		return $success;
	}
	...
	protected function deleteSql(&$rs, $where = "", $curfilter = TRUE)
	{
		...
		if (is_array($where))
			$where = $this->arrayToFilter($where);
		if ($rs) {
			if (array_key_exists('idusuarios', $rs))
				AddFilter($where, QuotedName('idusuarios', $this->Dbid) . '=' . QuotedValue($rs['idusuarios'], $this->idusuarios->DataType, $this->Dbid));
		}
		$filter = ($curfilter) ? $this->CurrentFilter : "";
		AddFilter($filter, $where);
		if ($filter <> "")
	...
	protected function loadDbValues(&$rs)
	{
		...
		$this->idusuarios->DbValue = $row['idusuarios'];
		$this->nombrecompleto->DbValue = $row['nombrecompleto'];
	...
	public function getRecordFilter($row = NULL)
	{
		...
		$val = is_array($row) ? (array_key_exists('idusuarios', $row) ? $row['idusuarios'] : NULL) : $this->idusuarios->CurrentValue;
		if (!is_numeric($val))
			return "0=1"; // Invalid key
		if ($val == NULL)
			return "0=1"; // Invalid key
		else
			$keyFilter = str_replace("@idusuarios@", AdjustSql($val, $this->Dbid), $keyFilter); // Replace key value
		return $keyFilter;
	}
	...
	{
		$json = "";
		$json .= "idusuarios:" . JsonEncode($this->idusuarios->CurrentValue, "number");
		$json = "{" . $json . "}";
		if ($htmlEncode)
	...
	public function keyUrl($url, $parm = "")
	{
		...
		if ($parm <> "")
			$url .= $parm . "&";
		if ($this->idusuarios->CurrentValue != NULL) {
			$url .= "idusuarios=" . urlencode($this->idusuarios->CurrentValue);
		} else {
	...
	public function getRecordKeys()
	{
		...
		if (Param("key_m") !== NULL) {
			...
			if (Param("idusuarios") !== NULL)
				$arKeys[] = Param("idusuarios");
	...
	public function getFilterFromRecordKeys()
	{
		...
		foreach ($arKeys as $key) {
			...
			$this->idusuarios->CurrentValue = $key;
	...
	{
		$this->idusuarios->setDbValue($rs->fields('idusuarios'));
	...
	public function renderListRow()
	{
		...
		// idusuarios
		$this->idusuarios->LinkCustomAttributes = "";
		$this->idusuarios->HrefValue = "";
		$this->idusuarios->TooltipValue = "";
	...
	public function renderEditRow()
	{
		...
		// idusuarios
		$this->idusuarios->EditAttrs["class"] = "form-control";
		$this->idusuarios->EditCustomAttributes = "";
		$this->idusuarios->EditValue = $this->idusuarios->CurrentValue;
		$this->idusuarios->ViewCustomAttributes = "";
		...
		$this->nombrecompleto->EditValue = $this->nombrecompleto->CurrentValue;
		...
		if (!$doc->ExportCustom) {
			...
			if ($doc->Horizontal) { // Horizontal format, write header
				...
				if ($exportPageType == "view") {
					$doc->exportCaption($this->idusuarios);
					$doc->exportCaption($this->nombrecompleto);
					$doc->exportCaption($this->correo);
					$doc->exportCaption($this->contrasenia);
		...
		while (!$recordset->EOF && $recCnt < $stopRec) {
			...
			if ($recCnt >= $startRec) {
				...
				if (!$doc->ExportCustom) {
					...
					if ($exportPageType == "view") {
						$doc->exportField($this->idusuarios);
						$doc->exportField($this->nombrecompleto);
						$doc->exportField($this->correo);
						$doc->exportField($this->contrasenia);
	...
	public function prepareRegisterEmail($row = NULL, $langId = "")
	{
		...
		$receiverEmail = $row == NULL ? $this->correo->CurrentValue : $row['correo'];
		...
		$email->replaceContent('<!--FieldCaption_nombrecompleto-->', $this->nombrecompleto->caption());
		$email->replaceContent('<!--nombrecompleto-->', $row == NULL ? strval($this->nombrecompleto->FormValue) : $row['nombrecompleto']);
		$email->replaceContent('<!--FieldCaption_correo-->', $this->correo->caption());
		$email->replaceContent('<!--correo-->', $row == NULL ? strval($this->correo->FormValue) : $row['correo']);
		$email->replaceContent('<!--FieldCaption_contrasenia-->', $this->contrasenia->caption());
		$email->replaceContent('<!--contrasenia-->', $row == NULL ? strval($this->contrasenia->FormValue) : $row['contrasenia']);
